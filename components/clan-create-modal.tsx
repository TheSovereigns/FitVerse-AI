        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-bold uppercase tracking-wider text-foreground/40 mb-1.5 block">
                  {isEnglish ? "Clan Name" : "Nome do Clan"}
                </label>
                <input value={name} onChange={(e) => setName(e.target.value)} maxLength={30}
                  placeholder={isEnglish ? "e.g., VyseFit United" : "ex.: VyseFit Unidos"}
                  className="h-12 w-full rounded-xl border border-border bg-card px-4 text-sm text-foreground placeholder:text-foreground/20 outline-none focus:border-brand/30 transition-colors" />
              </div>
              <div>
                <label className="text-[10px] font-bold uppercase tracking-wider text-foreground/40 mb-1.5 block">
              ...
              <div>
                <label className="text-[10px] font-bold uppercase tracking-wider text-foreground/40 mb-2 block">
                  ...
                  {[{ val: true, icon: Globe, label: isEnglish ? "Public" : "Publico" },
                    ...
                    <button key={String(val)} onClick={() => setIsPublic(val)}
                      className={cn("flex-1 flex items-center justify-center gap-2 h-11 rounded-xl text-xs font-bold border transition-all",
                        isPublic === val ? "bg-brand/10 text-brand border-brand/20" : "text-foreground/40 border-border hover:text-foreground/60")}>
            ...
            <button onClick={handleCreate} disabled={!name.trim() || isLoading}
              className="mt-6 h-12 w-full rounded-xl bg-brand text-foreground text-sm font-bold hover:bg-brand/90 transition-colors disabled:opacity-40 flex items-center justify-center gap-2">
